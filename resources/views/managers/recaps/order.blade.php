            ...
            <table class="table table-condensed table-borderless table-sm">
                ...
                <tr>
                    <td style="text-align: left">
                     <span id="product-price-{{$item->unique_id}}" class="product-price">Rp {{number_format($item->price,0,",",".")}}</span>
                 </td>
                 <td style="text-align: right">

                    <div id="product-add-{{$item->unique_id}}" class="product-add">
                        <button type="button" class="btn btn-success btn-sm" onclick="addButton('{{$item->unique_id}}', '{{$item->name}}', '{{$item->price}}')">+ Tambah</button>
                    </div>
                    <div id="product-order-{{$item->unique_id}}" class="product-order">
                        <div class="product-order-container">
                            <button type="button" class="btn btn-sm"  data-toggle="modal" data-target="#notesModal"  data-backdrop="static" data-keyboard="false" onclick="modalNotes('{{$item->unique_id}}')"><i class="fa fa-pencil"></i></button>
                            <button type="button" class="btn btn-danger btn-sm"  onclick="minusButton('{{$item->unique_id}}', '{{$item->name}}', '{{$item->price}}')">-</button>
                            <button>
                                <span id="product-quantity-{{$item->unique_id}}">0</span>
                            </button>
                            <button type="button" class="btn btn-success btn-sm"  onclick="plusButton('{{$item->unique_id}}', '{{$item->name}}', '{{$item->price}}')">+</button>
                        </div>
                    </div>
                </td>
            </tr>
...
<form id="order-form" method="POST" action="/manager/recap/submit" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    <div id="cart-full">
            <div id="cart-container">
                <div class="flex-container">
                  <div>
                   <input id="customer" type="hidden" class="form-custom form-control" name="customer" value="REKAP" placeholder="Pemesan" required>

                   <input id="tenant_user_id" type="hidden" class="form-custom form-control" name="tenant_user_id" value="{{$tenant_user_id}}" placeholder="Pemesan" required>
               </div>
               <div>
                <input id="tableNumber" type="hidden" class="form-custom form-control" name="tableNumber" value="" placeholder="Nomor" required>
            </div>
        </div>
...
<div id="notesModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-footer">
        ...
        onclick="saveNotes()"  data-dismiss="modal">
        Simpan
    </button>
</div>
</div>
